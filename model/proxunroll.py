...
class ProxUnroll(nn.Module):
    ...
    def reconstruct(self, measurements, image_size, cr, stages=None, use_memory=True,
                    sensing_matrices=None):
        """Reconstruct from [batch, measured_height, measured_width], without GT.

        Returns one image per restorer stage, with shape [stage, batch, h, w].
        Shorter trajectories and disabling memory are inference interventions,
        not separately trained ablations.
        """
        count = self.stages if stages is None else stages
        if not isinstance(count, int) or not 1 <= count <= min(self.stages, len(self.rho)):
            raise ValueError('stages must be between 1 and the trained stage count')
        h, w = image_size
        if sensing_matrices is None:
            H, W, HT, WT = self.measurement_matrices(h, w, cr)
        else:
            if not 0 < cr <= 1:
                raise ValueError('cr must be in (0, 1]')
            H, W = sensing_matrices
            expected = (int(np.ceil(h * np.sqrt(cr))), int(np.ceil(w * np.sqrt(cr))))
            if (H.ndim != 2 or W.ndim != 2 or H.shape[1] != h or W.shape[1] != w
                    or not 1 <= H.shape[0] <= expected[0]
                    or not 1 <= W.shape[0] <= expected[1]):
                raise ValueError('sensing matrices do not match image_size and cr')
            H, W = H[None, None].to(measurements), W[None, None].to(measurements)
            HT, WT = H.transpose(-2, -1), W.transpose(-2, -1)
        if measurements.ndim != 3 or measurements.shape[-2:] != (H.shape[-2], W.shape[-2]):
            raise ValueError('measurement shape does not match image_size and cr')
        Y = measurements.unsqueeze(1)
        X = HT @ Y @ W
        ...
        for i in range(count):
            if self.solver == 'hqs':
                ...
                X, memory = self.restorer(Z, memory if use_memory else None)
            else:
                Z = self.prox_f(X - U, Y, H, W, HT, WT, self.rho[i])
                X, memory = self.restorer(Z + U, memory if use_memory else None)
                U = U + Z - X
